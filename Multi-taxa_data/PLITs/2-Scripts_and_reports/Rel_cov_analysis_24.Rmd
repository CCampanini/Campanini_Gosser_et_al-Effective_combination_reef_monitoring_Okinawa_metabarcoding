...
ggplot(nmds_df, aes(NMDS1, NMDS2))+
  ...
  guides(color= guide_legend(order=1, title = "Site", ncol =2))+
  new_scale_color() +
  geom_mark_ellipse(aes(fill=Pressure, color=Pressure), alpha=0.2)+ # https://luisdva.github.io/rstats/Grouping-points/
  scale_color_manual(values= c("#009F81", "#FFB331","#9F1111"))+
  scale_fill_manual(values= c("#009F81", "#FFB331","#9F1111"))+
  theme_classic()+
  ggtitle("nMDS of relative benthic cover")+
...
ggplot(nmds_df, aes(NMDS1, NMDS2))+
  ...
  theme(plot.title = element_text(size=14, face="bold",hjust=.5),
        legend.title = element_text(color = "black", size = 12),
        legend.text = element_text(size=10))+
  scale_x_continuous(limits = c(-1.2,1.8))+
...
palette_check(pal_fabian, plot = TRUE)
palette_check(pal12, plot = TRUE)
...
ggplot(nmds_df, aes(NMDS1, NMDS2))+
  stat_ellipse(geom = "polygon", aes(fill = Pressure), alpha = 0.2) +
  geom_point(data=nmds_df, aes(NMDS1, NMDS2, color=Site, shape=Pressure),size=2.5, alpha = 0.8)+##separates overlapping points+
  scale_fill_manual(values= c("#009F81", "#FFB331","#9F1111"))+
  scale_color_manual(values= pal_fabian)+
...
```{r}
mantel(Dist.km, cov_bray)
```

Plot
```{r}
matrix.dist <- data.frame(x = melt(as.matrix(Dist.km))$value, 
                          y = melt(as.matrix(Ok.jac))$value) ## Prepare data for graphic
