...
library(reshape2) #needed for dcast function
mean_rel_cov_pressure <- dcast(mean_rel_cov_pressure, 
                               Site ~ Pressure)

mean_rel_cov_pressure$Pressure<-ordered(mean_rel_cov_pressure$Pressure,
                                    levels=c("Low","Medium","High"))

mean_rel_cov_site <- mean_rel_cov_site[,c(1,11,2:10)] #re-ordering columns

mean_rel_cov_site$Pressure <- ordered(mean_rel_cov_site$Pressure,levels
                                      =c("Low","Medium","High"))
...
### Question 4: is there a significant difference in relative benthic cover among sites within pressure levels?
...
K. Clarke. Permanova+ for primer: Guide to software and statistical
...
```{r}
set.seed(69) 
densityplot(permustats(relcov_pressure_pmv)) #density plot of permutation results to to check if assumptions are met?
