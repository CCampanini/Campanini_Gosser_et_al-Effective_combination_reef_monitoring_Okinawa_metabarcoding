...
func_ndms_ggplot<-ggplot(nmds_df, aes(NMDS1, NMDS2))+
  ...
  geom_mark_ellipse(aes(fill=Pressure, color=Pressure), expand = 0)+ # https://luisdva.github.io/rstats/Grouping-points/
  theme_classic()+
  ...
  annotate("text", x=-1.49, y=-0.47,
           label=bquote('Non-metric fit,'~ R^2~"= 0.921"),
...
box+
  annotate(geom = "table", x=9,y = 0.43, label = list(distcent_df_mean), 
            vjust = 1, hjust = 0,
            fill="grey96")+
