...
mol_raw_df$site <-ordered(mol_raw_df$site,levels=c("G1", "G2", "AW", "SN", "SK", "MI", "YO","ZA", "ZP", "HI", "S1", "S2"))
...
mol_class <- mol_class[,c(5,1,6,7,2:4)]
library(janitor) #to use the tabyl function

mol_class$Cephalopoda <- NULL

mol_class$site <-ordered(mol_class$site,levels=c("G1", "G2", "AW", "SN", "SK", "MI", "YO","ZA", "ZP", "HI", "S1", "S2"))
...
mol_mean <- mol_class %>%
  group_by(site_depth) %>% 
  summarise(Biv_mean=round(mean(Bivalvia),0),
...
ech_raw_df$site <-ordered(ech_raw_df$site,levels=c("G1", "G2", "AW", "SN", "SK", "MI", "YO","ZA", "ZP", "HI", "S1", "S2"))
...
ech_class <- ech_class[,c(7,1,8,9,2:6)]

ech_class$site <-ordered(ech_class$site,levels=c("G1", "G2", "AW", "SN", "SK", "MI", "YO","ZA", "ZP", "HI", "S1", "S2"))
...
AW_S3 <- c("AW_S3",rep(NA,5))

mol_class[nrow(mol_class) + 3,] <- list("AW_S3",rep(NA,5),
                                        "G1_D3",rep(NA,5),
                                        "S2_D1",rep(NA,5))
...
inv_df <- merge(mol_class, ech_class, by=c("transect","site","depth","site_depth"), all = T)

inv_mat <- inv_df[,-c(1:4)]
...
Sites <- inv_bd$group
distcent_df[,2]<-factor(distcent_df[,2],levels=c("AW", "G1", "G2", "HI", "MI", "S1", "S2", "SK", "SN", "YO", "ZA", "ZP"))
