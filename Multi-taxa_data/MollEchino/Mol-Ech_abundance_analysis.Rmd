...
names(mol_raw) = c("awa", "g1", "g2", "hi", "mi", "s1", "s2", "sk", "sn", "yo", "za", "zp")
...
mol_class$depth <- gsub("[[:digit:]]", "", mol_class$depth_rep) #remove the number from depth_rep
...
mol_class <- mol_class[,c(4,1,5:7,2:3)]

mol_class[,3:4] <- lapply(mol_class[,3:4], as.factor)
```
...
mol_mean <- mol_class %>%
  group_by(site_depth) %>% 
  mutate(Biv_mean=round(mean(Bivalvia),0),
...
```{r}
library(reshape2)
mol_mean_long <- melt(mol_mean[,1:3], id.vars = c("site_depth"),
  variable.name = "class", 
  value.name = c("mean"))

mol_sd_long <- melt(mol_mean[,c(1,4,5)], id.vars = c("site_depth"),
  variable.name = "class", 
  value.name = c("sd"))
...
mol_mean_bar <- mol_mean_bar %>% 
  separate(site_depth, c("site", "depth")) %>% #split the column site_depth in 2
  mutate(site_depth = mol_mean_bar$site_depth ) %>% #re-integrate the original site_depth column
...
mol_mean_bar <- mol_mean_bar[,c(6,1:5)]
...
names(ech_raw) = c("awa", "g1", "g2", "hi", "mi", "s1", "s2", "sk", "sn", "yo", "za","zp")
...
ech_class <- ech_raw_df[!is.na(ech_raw_df$CLASS),] %>% tabyl(transect, CLASS, show_missing_levels = FALSE)
...
ech_class[,3:4] <- lapply(ech_class[,3:4], as.factor)
```
...
ech_mean <- ech_class %>%
  group_by(site_depth) %>% 
  summarise(Ast_mean=round(mean(Asteroidea),0),
...
```{r}
library(reshape2)
ech_mean_long <- melt(ech_mean[,1:6], id.vars = c("site_depth"),
  variable.name = "class", 
  value.name = c("mean"))

ech_sd_long <- melt(ech_mean[,c(1,7:11)], id.vars = c("site_depth"),
  variable.name = "class", 
  value.name = c("sd"))
...
ech_mean_bar <- ech_mean_bar %>% 
  separate(site_depth, c("site", "depth")) %>% #split the column site_depth in 2
  mutate(site_depth = ech_mean_bar$site_depth ) %>% #re-integrate the original site_depth column
...
ech_mean_bar <- ech_mean_bar[,c(6,1:5)]
...
inv_bar$class <- factor(inv_bar$class,levels=c("Bivalvia", "Gastropoda","Asteroidea","Crinoidea","Echinoidea","Holothuroidea","Ophiuroidea"))

inv_bar$site <- factor(inv_bar$site, levels=c("HI", "S1", "S2", "ZP", "ZA", "YO", "MI","SK", "SN", "AW", "G1", "G2"))
...
ggplot(inv_bar, aes(x=depth, y=mean, fill=class, label=mean, color=phylum)) +
  ...
  geom_errorbar(aes(ymin=ifelse(mean > 0, 0, 0), 
                    ymax=ifelse(mean > 0, mean+sd, 0)), 
