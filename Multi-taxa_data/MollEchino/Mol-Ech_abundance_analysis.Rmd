...
Plot filled with colors by pressure
...
```{r}
mol_ech_abundance_pressure_barplot <- ggplot(subset(inv_bar_pressure, inv_bar_pressure$class != "Ophiuroidea"),
       aes(x=depth, y=mean, 
           fill=pressure, 
           label=mean, 
           color=pressure)) +
...
mol_ech_abundance_pressure_barplot
```
...
mol_ech_ndms <- ggplot(nmds_df, aes(NMDS1, NMDS2))+
...
```{r}
fig4 <- cowplot::  ggdraw() +
  draw_plot(mol_ech_abundance_pressure_barplot, x = 0, y = 0, width = .4, height = 1) +
  draw_plot(mol_ech_ndms, x = .4, y = 0, width = 0.6, height = 1) +
