...
Split Echinoidea and all the other classes in 2 different bar plots

```{r}
ggplot(subset(inv_bar,inv_bar$class != "Echinoidea" & 
                inv_bar$class != "Crinoidea"), 
...
ggplot(subset(inv_bar,inv_bar$class == "Echinoidea" |
  ...
  labs(x="Depth",
       y="Mean abundance by class")+
  scale_fill_manual(values=c("#CC79A7","orchid3"))+
...
ggplot(subset(inv_bar, inv_bar$class != "Ophiuroidea"),
       aes(x=site_depth, y=mean, fill=class, label=mean, color=phylum)) +
  geom_bar(aes(alpha=phylum),position="dodge",stat="identity") +
  geom_errorbar(aes(ymin=mean, ymax=mean+sd), width=.2,
                 position=position_dodge(.9))+
  facet_grid( ~ class, scales = "free")+ # https://stackoverflow.com/questions/18046051/setting-individual-axis-limits-with-facet-wrap-and-scales-free-in-ggplot2
  theme_classic()+
  labs(x="Depth",
       y="Mean abundance by class")+
  scale_fill_manual(values=invPalette)+
...
```{r}
ggplot(subset(inv_bar, inv_bar$class != "Ophiuroidea"),
       aes(x=site, y=mean, fill=class, label=mean, color=phylum)) +
...
ggplot(subset(inv_bar, inv_bar$class != "Ophiuroidea"),
       aes(x=site, y=mean, fill=class, label=mean, color=phylum)) +
  geom_bar(aes(alpha=depth),position=position_dodge(),stat="identity") +
  geom_errorbar(aes(ymin=mean, ymax=mean+sd), width=.2,
                 position=position_dodge(.9))+
  facet_grid( ~ class, scales = "free")+ # https://stackoverflow.com/questions/18046051/setting-individual-axis-limits-with-facet-wrap-and-scales-free-in-ggplot2
  theme_classic()+
