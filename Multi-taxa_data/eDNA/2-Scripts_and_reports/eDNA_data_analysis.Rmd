...
# 
combined_counts_bar <- ggplot(combined_counts, aes(x = `Taxonomic rank`)) +
...
ggplot(Ok2, aes(x = Sample, y = Abundance, fill = Phylum)) +
  ...
  paletteer:: scale_fill_paletteer_d("ggthemes::Tableau_20") +  # Using viridis color palette
...
Add faceting by impact

```{r}
esv_counts_per_site$impact <- ordered(esv_counts_per_site$impact, levels = c( "low", "medium", "high"))
levels(esv_counts_per_site$impact )
...
unique_ESVs_site_bar <- ggplot(esv_counts_per_site, aes(x = Location, y = ESV_Count, fill = Phylum)) +
  ...
  paletteer:: scale_fill_paletteer_d("ggthemes::Tableau_20") +
  labs(y = "Count of Unique ESVs",fill = "Phylum", x= "Site") +
  theme_classic() +
...
```{r}
#tableau_20_palette <- paletteer::paletteer_d("ggthemes::Tableau_20")
...
edna_phylo_tree <- edna_phylo_tree +  geom_cladelab(data = filtered_Ok_tree_df, mapping = aes(node = node, label = label), 
                      ...
                      extend = 0.3,
                      offset.text = 1, 
                      barcolor = c("#FF9D9AFF","#BAB0ACFF","#4E79A7FF","#B07AA1FF","#499894FF","#E15759FF","#F28E2BFF", "#8CD17DFF","#A0CBE8FF", "#59A14FFF","#79706EFF", "#D37295FF", "#D4A6C8FF","#86BCB6FF", "#9D7660FF", "#D7B5A6FF",  "#B6992DFF", "#FABFD2FF", "#FFBE7DFF", "#F1CE63FF" ))
...
ggsave("edna_phylo_tree.png", 
       ...
       units ="in", width = 9, height = 9, dpi = 600,
       path = here::here("Multi-taxa_data","eDNA","4-Visualization_outputs"))
```
...
pie_chart_total <- ggplot(family_counts_phylum, 
  ...
  paletteer:: scale_fill_paletteer_d("ggthemes::Tableau_20")+  # Specify the color palette
  theme_void()
  theme_void()
...
pie_charts <- family_counts %>%
  ...
  map(~ {
    ...
    ggplot(site_data, aes(x = "", y = TotalFamilies.x, fill = Phylum)) +
      ...
      scale_fill_manual(values = ifelse(site_data$TotalFamilies.x > site_data$Threshold, c("#4E79A7FF", "#A0CBE8FF", "#F28E2BFF", "#FFBE7DFF", "#59A14FFF", "#8CD17DFF", "#B6992DFF", "#F1CE63FF", "#499894FF", "#86BCB6FF", "#E15759FF", "#FF9D9AFF", "#79706EFF", "#BAB0ACFF", "#D37295FF", "#FABFD2FF", "#B07AA1FF", "#D4A6C8FF", "#9D7660FF", "#D7B5A6FF"), "grey50")) +
      theme_void() +
...
taxonomy <- as.data.frame(phyloseq::tax_table(Ok))
