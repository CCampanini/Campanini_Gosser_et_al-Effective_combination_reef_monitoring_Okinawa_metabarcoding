...
```{r}
Ok_tree <- ape::read.tree(here("Multi-taxa_data/eDNA/2-Clean_data/eDNAOkinawaTreenew.tre"))
```
...
plot +  geom_cladelab(data = filtered_Ok_tree_df, 
...
posthoc_richness_df <- bind_rows(posthoc_richness) #Convert list to dataframe and print
...
### Extracting relevant values and create a data frame
...
plot_grid(plot.shan_impact + theme(legend.position = "none"),
          plot.rich_impact + theme(legend.position = "none"),
          plot.even_impact + theme(legend.position = "none"), 
          ncol = 3)
```

## Kruskal-Wallis test for impact
...
### Kruskal-Wallis and post-hoc Dunn test for Shannon index

```{r}
kw_shannon_site <- kruskal.test(shannon ~ Location, data = alpha)
kw_shannon_site
posthoc_shannon_site <- dunn.test(alpha$shannon, 
                                  alpha$Location, 
                                  method = "bonferroni")
```

```{r}
kw_shannon_pressure <- kruskal.test(shannon ~ impact, data = alpha)
kw_shannon_pressure
posthoc_shannon_pressure <- dunn.test(alpha$shannon, 
                                      alpha$impact, 
                                      method = "bonferroni")
```

### Kruskal-Wallis and post-hoc Dunn test for Pielou's index


```{r}
kw_pielou_site <- kruskal.test(pielou ~ Location, data = alpha)
kw_pielou_site
posthoc_pielou_site <- dunn.test(alpha$pielou, 
                                  alpha$Location, 
                                  method = "bonferroni")
```

```{r}
kw_pielou_pressure <- kruskal.test(pielou ~ impact, data = alpha)
kw_pielou_pressure
posthoc_pielou_pressure <- dunn.test(alpha$pielou, 
                                      alpha$impact, 
                                      method = "bonferroni")
```
...
Code used to execute the the nmds and save the output 

```{r, eval = F}
set.seed(69)
Ok_nmds_2 = metaMDS(otu_table(Ok), 
                  distance = "jac", 
                  k = 2, # n of dimensions
                  maxit = 999, # Max n of iterations in the single NMDS run
                  trymax = 20, # Min and max n of random starts in search of stable solution.
                  wascores = TRUE, # Computes Weighted Averages scores of species for ordination configuration or for environmental variables (T is default)
                  tidy = TRUE # Return scores that are compatible with ggplot2
                  )

Ok_nmds_3 = metaMDS(otu_table(Ok), 
                  distance = "jac", 
                  k = 3, # n of dimensions
                  maxit = 999, # Max n of iterations in the single NMDS run
                  trymax = 20, # Min and max n of random starts in search of stable solution.
...
Load file with nmds data
```{r}
Ok_nmds <- readRDS(file = here("Multi-taxa_data",
                    "eDNA",
                    "4-Output",
                    "Ok_nmds.Rds"))
```
...
relfunc_dij=metaMDSdist(otu_table(Ok), trace=FALSE)
set.seed(69)
for(i in seq_along(k_vec)) {
  edna_nmds_br_loop=metaMDSiter(relfunc_dij, k=i, trace=FALSE)
  stress[i]=edna_nmds_br_loop$stress
}
...
```{r}
Ok_data_scores <- as.data.frame(scores(Ok_nmds$points))
class(scores(Ok_nmds$points))
str(Ok_nmds)
```
```{r}
Ok_data_scores <- as.data.frame(scores(Ok_nmds$species))
str(Ok_nmds)
```

Add columns to data frame 
```{r}
Ok_data_scores$depth = Okinawa_env$depth
Ok_data_scores$Location = Okinawa_env$Location
Ok_data_scores$impact = Okinawa_env$impact
```


```{r}
plot.nmds = ggplot(Ok_data_scores, aes(x = MDS1, y = MDS2)) +
...
```{r}
esv_mat_trans <- t(esv_mat)
Ok_nmds = metaMDS(esv_mat_trans, 
                  distance = "jac", 
                  k = 3, 
                  maxit = 999, 
                  trymax = 20, 
...
ggplot(Ok_data.scores, aes(MDS1, MDS2))+
  geom_point(data=Ok_data.scores, aes(MDS1, MDS2, color=Location, shape=depth),size=2.5)+##separates overlapping points
  scale_color_manual(values= pal12)+
  guides(color= guide_legend(order=1, title = "Site", ncol =2))+
  new_scale_color() +
  geom_mark_ellipse(aes(fill=impact, color=impact), 
                    expand = 0, alpha=0.2)+ # https://luisdva.github.io/rstats/Grouping-points/
